...
class LT_Dataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.transform is not None and len(self.transform) == 3:
        ...
        if self.transform is not None and len(self.transform) == 2: # 测试集
            ...
            # img_weak = 1
            img_strong = 1
            img_base = self.transform[1](sample)
            
        return img_weak, img_strong,img_base, label
...
def Load_ImageNet(data_root, dataset, phase, batch_size, num_workers=4, shuffle=True):
    ...
    if dataset == 'ImageNet_LT':
        ...
        if phase == 'train':
            # set_ = LT_Dataset(data_root, txt, transform=[data_transforms_ImageNet['base'],
